...
首次使用、重新安装或对流程不确定时，先完整阅读 `references/example.md`，按其中的输入、确认点、切点记录、产出数量和命名方式执行。案例秒数只展示数据结构，禁止套用到新素材。
...
详细字段、目录建议和命令示例见 `references/workflow.md`；完整虚构案例见 `references/example.md`。
